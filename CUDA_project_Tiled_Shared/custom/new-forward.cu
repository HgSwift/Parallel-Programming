...
__global__ void conv_forward_kernel(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    */
    ...
    b = blockIdx.x;
    m = blockIdx.y;
    int h_b = (blockIdx.z / W_grid)*TILE_WIDTH;
    int w_b = (blockIdx.z % W_grid)*TILE_WIDTH;
    int h0 = threadIdx.y;
    int w0 = threadIdx.x;
    int col0 = threadIdx.y + m*TILE_WIDTH;
    int row0 = threadIdx.x + b*TILE_WIDTH;
    //__shared__ float tileX[TILE_WIDTH + K - 1][TILE_WIDTH + K - 1];
    //__shared__ float tileW[TILE_WIDTH + K - 1][TILE_WIDTH + K - 1];
    h = h0 + h_b;
    w = w_b + w0;
    ...
    for (c = 0; c < C; c++) { 
        ...
        for(int i = h; i<h_b + X_tile_width; i += TILE_WIDTH ){
            for(int j = w; j<w_b + X_tile_width; j += TILE_WIDTH ){
                if(i<H && j<W){
                    X_shared[(i - h_b)*X_tile_width + j - w_b] = x4d(b,c,i,j);
                }
            }
        }
        __syncthreads();
